...
void threadStart(int id, mutex& qm, mutex& hm, unordered_map<int, int>& hash, queue<int>& tasks) {
	while (!tasks.empty()) {
		int num;
		int digit;
		if (tasks.size() == 0) {
...
int main() {
	...
	int totthread = thread::hardware_concurrency();
	queue <int> tasks;
	mutex qm;
